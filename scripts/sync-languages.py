...
if __name__ == "__main__":
    ...
    # When invoked as part of cwm-build-tools, the consuming project's
    # current working directory is the project root. The previous behavior
    # (project_root = parent of the script directory) only worked when the
    # script lived at <project>/build/. Honor an explicit --project-root if
    # given, otherwise default to CWD.
    project_root = os.getcwd()
    for _i, _a in enumerate(sys.argv):
        if _a == '--project-root' and _i + 1 < len(sys.argv):
            project_root = os.path.abspath(sys.argv[_i + 1])
            del sys.argv[_i:_i + 2]
            break
        if _a.startswith('--project-root='):
