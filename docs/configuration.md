...
| `lint` | Optional. `{ paths[], excludeDirs[], skip[], issueDigits }` — source roots the lint commands scan, and extra directory basenames to skip on top of `vendor/`, `node_modules/`, `build/`, `dist/` and the VCS directories. A git submodule is a separate repository and not this project's standard to enforce; name it in `excludeDirs`. Defaults to the conventional Joomla roots that exist. `skip[]` and `issueDigits` are `cwm-lint-comments`' equivalents — path fragments to skip (on top of `vendor/`, `node_modules/`, `.git/`, `tests/`, `libraries/`, `.min.`) and how many digits make a token an issue number (default 4; three collide with CSS hex colours, so a repo numbering in the hundreds must lower it). |
| `testSite` | Optional. `{ reset: { elements[], elementPatterns[], retain[], tablePrefixes[], components[], menuLinkPatterns[], typeAliasPatterns[], modulePatterns[], updateSiteNamePatterns[], actionLogPatterns[], directories[], fileGlobs[], groups{} } }` — what `cwm-reset-testsite` removes from a `role = test` install, and what it must not. `retain[]` overrides a family match and is printed and re-checked after every run, so "this survived on purpose" is distinguishable from "this survived because nobody looked". An `elements[]` entry may be a plain string (match the element wherever it is) or `{ element, type?, folder? }` to name one of two plugins sharing an element. `groups{}` holds optional families enabled with `--with <name>` — for a stack shared with another extension, which must not be removed unasked. |
| `schemaReplay` | Optional. `{ prefix?, targets[] }` — what `cwm-schema-replay` executes. Each target is `{ name, manifest, baseline, from }`, where `baseline` is a path or a list of paths applied in order and `from` is the version that baseline represents. The baseline is a **site's** schema, not the extension's install SQL: migrations write to core tables no manifest creates. See [the schema replay baseline](commands.md#the-schema-replay-baseline). |
