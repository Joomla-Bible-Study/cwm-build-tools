...
| `testSite` | Optional. `{ reset: { elements[], elementPatterns[], retain[], tablePrefixes[], components[], menuLinkPatterns[], modulePatterns[], updateSiteNamePatterns[], actionLogPatterns[], directories[], fileGlobs[] } }` — what `cwm-reset-testsite` removes from a `role = test` install, and what it must not. `retain[]` overrides a family match and is printed and re-checked after every run, so "this survived on purpose" is distinguishable from "this survived because nobody looked". |
